...
export function App() {
  ...
  useEffect(() => {
    document.title = doc ? doc.fileName : 'Nawa Docs'
  }, [doc])
    ...
    () =>
      window.desktop.onZoteroRequest(async (request) => {
        try {
          ...
          if (!activeEditor) throw new Error('No active Nawa document')
          const controller =
  ...
  // Nawa CLI (`open --block`, `selection`): the shell evaluates this hook
  useEffect(() => {
