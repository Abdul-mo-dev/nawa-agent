...
/**
 * Every launch: record where the Nawa launcher lives so agents can find it
...
export function installCliLinkBestEffort(settingsPath: string): void {
  ...
  try {
    if (isEphemeralInstall(process.resourcesPath, process.env)) {
      ...
      console.log('[Nawa] cli link skipped: app runs from a temporary mount')
      return
    }
    ...
    console.log(
      `[Nawa] cli link: ${outcome.status}${outcome.location ? ` (${outcome.location})` : ''}`,
    )
    ...
    console.warn('[Nawa] cli link failed:', err instanceof Error ? err.message : err)
  }
}
...
/** Same directory the Nawa CLI uses for auth.json and its audit log. */
...
/** One line, the directory holding Nawa / Nawa.cmd; rewritten only when it changed. */
