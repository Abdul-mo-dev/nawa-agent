...
function noMatch(target: string, documents: readonly OpenDocumentTab[]): Error {
  if (documents.length === 0) {
    return new Error(`no document is open in Nawa, so "${target}" cannot be resolved`)
  }
  ...
  return [
    {
      name: 'open_documents',
      description:
        'Work with the documents the user currently has open in Nawa tabs — not just the ' +
      ...
      handler: async (args) => {
        if (!control) {
          throw new Error('Nawa is not running, so its open documents are unreachable')
        }
