 ...
 * bundled `Nawa` CLI — the same engines the app ships — instead of
...
export interface DocToolDeps {
  ...
  background?: boolean
  /** open a file in the Nawa UI; wired in M4 (optional in tests) */
  ...
  extraFormats?: string[]
  /** the bundled Nawa CLI, used by the headless tools */
  cli?: CliRunner
  /**
...
function createHeadlessDocxTool(deps: DocToolDeps): McpToolDefinition {
  return {
    ...
    description:
      ...
      'the Nawa CLI use. Returns the absolute path of the written file.',
    inputSchema: {
...
export function createDocumentTools(deps: DocToolDeps, host: SessionHost): McpToolDefinition[] {
  return [
    ...
    },
    {
      ...
      description: 'Open an existing file in the running Nawa app, focusing its tab.',
      inputSchema: {
    ...
    },
    {
      name: 'get_app_info',
      description:
        'Report Nawa version, the default save folder, the document formats this server can ' +
