 ...
 * - headless `create_xlsx`: a row matrix handed to the bundled `Nawa` CLI
...
export interface SheetsToolDeps {
  ...
  /** the bundled Nawa CLI, used by the headless tool */
  cli?: CliRunner
  /**
...
function createHeadlessXlsxTool(deps: SheetsToolDeps): McpToolDefinition {
  return {
    ...
    description:
      ...
      'formula (the Nawa CLI evaluates it and stores the cached value). Returns the absolute ' +
