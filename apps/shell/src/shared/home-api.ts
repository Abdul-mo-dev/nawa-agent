...
export interface HomeApi extends WorkspaceApi {
  ...
  /** effective default save folder for new/untitled files (configured in userData/app-settings.json, falls back to <Documents>/Nawa) */
  ...
  /** capped text slice of one workspace file for directory chat (stays inside the folder root) */
  readWorkspaceFile(path: string, maxChars: number): Promise<WorkspaceFileText>
  /** start an AI stream for directory chat; chunks arrive on onAiStreamChunk */
  aiStream(request: AiStreamRequest): Promise<void>
  /** abort a running directory-chat stream */
  aiStreamCancel(requestId: string): Promise<void>
  /** subscribe to AI stream chunks; returns unsubscribe */
  onAiStreamChunk(handler: (chunk: AiStreamChunk) => void): () => void
...
/** capped text slice of one workspace file for directory chat */
...
export const HOME_CHANNELS = {
  ...
  readWorkspaceFile: 'home:read-workspace-file',
} as const
