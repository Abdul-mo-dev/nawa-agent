...
import type { AiSettings, AiStreamChunk, CodexModelCatalog } from '@genoffice/ai-provider/browser'
...
const homeApi: HomeApi = {
  ...
  async readWorkspaceFile(path, maxChars) {
    if (typeof path !== 'string' || !path) throw new Error('Invalid file path.')
    const size =
      typeof maxChars === 'number' && Number.isFinite(maxChars)
        ? Math.min(maxChars, 48_000)
        : 12_000
    const result: unknown = await ipcRenderer.invoke(HOME_CHANNELS.readWorkspaceFile, path, size)
    return result as WorkspaceFileText
  },
  // Directory-chat streaming reuses the aggregated docs ai handlers (ai:stream
  // broadcasts chunks on ai:stream-chunk to the requesting webContents).
  ...
  onAiStreamChunk(handler) {
    const listener = (_event: Electron.IpcRendererEvent, chunk: unknown) => {
      if (chunk && typeof chunk === 'object' && 'requestId' in chunk) {
        handler(chunk as AiStreamChunk)
      }
    }
