...
const CODEX_BASE_INSTRUCTIONS =
  'You are the language-model backend embedded in Nawa. Never inspect or modify local files, run shell commands, browse, call MCP, use apps, or invoke any built-in Codex tool. The caller supplies the complete relevant conversation and a JSON Schema. Return exactly one assistant response matching that schema; Nawa itself executes document tools.'
...
class CodexAppServerClient {
  ...
  private async initialize(): Promise<void> {
    await this.requestWire('initialize', {
      clientInfo: { name: 'Nawa', title: 'Nawa', version: '0.1.0' },
  ...
  private onLine(line: string): void {
    ...
    if (typeof message.method === 'string' && message.id !== undefined) {
      // Nawa deliberately disables Codex-owned tools. Reply instead of
...
export function buildCodexAppServerPrompt(
  ...
  return [
    'Treat the payload below as the new Nawa conversation events for this turn and follow its system instruction.',
    'Do not use Codex tools. Nawa will execute only the tool calls returned in the required response schema.',
    'Put user-visible prose in text. Put requested Nawa tool calls in toolCalls; inputJson must be a JSON-encoded object matching the listed inputSchema. Use only listed tool names. If no tool is needed, return an empty toolCalls array.',
...
function incrementalMessages(
  ...
  // The assistant response is already native app-server history. Nawa's
...
const CODEX_PERMISSION_PROFILE = 'Nawa'
