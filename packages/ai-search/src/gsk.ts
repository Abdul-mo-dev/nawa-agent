...
function electronCompatArgs(): string[] {
  ...
  if (compatPath === undefined) {
    try {
      const dir = join(homedir(), '.Nawa', 'bin')
 ...
 * Priority: GSK_API_KEY env → Nawa's own key (bills to us via its
...
/**
 * Fires when the effective gsk key changes on disk — another Nawa-family
