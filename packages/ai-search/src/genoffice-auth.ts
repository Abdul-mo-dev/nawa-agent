/**
 * The app's Genspark identity: device-code login (office_addin_auth,
 ...
 * subprocesses) to the app. Stored in ~/.genoffice/auth.json, deliberately
...
/** The Nawa-named api key; '' when not signed in. Cached (invalidated by login/logout). */
...
/**
 * Starts the Nawa device-code login, cancelling a previous in-flight one
...
/**
 * Signs out of Nawa only: best-effort server-side revoke of the
 * Nawa key, then local removal. The shared gsk CLI login
