...
export const selectionCommand: CommandDef = {
  name: 'selection',
  summary:
    "What the user has selected in the Nawa editor showing this file (the user's own pointer for 'this one', 'here').",
  usage: 'selection <file> --json',
  async run(args, ctx) {
    const path = resolveInput(args.positionals[0], ctx)
    const endpoint = controlEndpoint(ctx.env)
    if (!endpoint) {
      throw new CliError(EXIT.app, 'Nawa is not running', undefined, {
