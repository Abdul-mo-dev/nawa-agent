...
export const openCommand: CommandDef = {
  name: 'open',
  summary:
    'Open a document in the Nawa app (starts the app if needed); with a target, also select that spot for the user.',
  usage:
  ...
  async run(args, ctx) {
    ...
    if (!endpoint) {
      ...
      if (!target) return { summary: `opening ${path} in Nawa`, detail: { app: launch } }
      endpoint = await waitForControlEndpoint(ctx.env, APP_START_TIMEOUT_MS)
      if (!endpoint) {
        throw new CliError(
          EXIT.app,
          'Nawa started but did not publish its control endpoint',
          undefined,
          {
    ...
    return {
      summary: target
        ? `${path}: showing ${describe(target)} in Nawa`
        : `opened ${path} in Nawa`,
