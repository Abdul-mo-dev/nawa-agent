...
/**
 * Files the running Nawa shell has open, from the registry it publishes
...
export function assertNotOpenInGui(abs: string, env: NodeJS.ProcessEnv): void {
  ...
  throw new CliError(
    ...
    { gui_pid: open.pid },
    {
      ...
      suggestion:
        'close the tab in Nawa first, or pass --force to write anyway (the editor may overwrite your change on its next save)',
    },
  )
}
