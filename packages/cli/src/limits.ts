...
export function assertPackageWithinLimits(path: string, ext: string): void {
  ...
  if (escaping) {
    throw refuse(
      ...
      { entry: escaping.name },
      'repair the file in Office or re-save it; Nawa does not open packages with unsafe entry paths',
    )
  }
  let total = 0
  ...
  for (const e of entries) {
    if (e.uncompressed > limits.maxPartBytes) {
      throw refuse(
        ...
        'remove or shrink the oversized part; Nawa refuses packages over detail.limit bytes',
      )
    }
    if (
  ...
  if (total > limits.maxTotalBytes) {
    throw refuse(
      ...
      'split the document or remove embedded media; Nawa refuses packages over detail.limit bytes',
    )
  }
