 ...
 * HTML and Markdown print, Word ↔ HTML) run in the Nawa binary itself through its
 * `--headless-export` entry: Dock hidden, no window, one export, exit. Nawa
...
export async function exportViaApp(
  ...
  if (!launch) {
    throw new CliError(EXIT.app, 'Nawa app not found (needed for this conversion)', {
  ...
  opts.log?.(`starting Nawa for ${target} export`)
  ...
  if (exported && (code === 0 || timedOut)) {
    if (timedOut) opts.log?.('export finished but Nawa had to be terminated on quit')
  ...
  const message =
    ...
    (tail ||
      (code === 0
        ? 'Nawa exited without writing the file; this Nawa version may not support --headless-export'
