 ...
 * apart from needing a DOM, so Nawa runs them under jsdom. They are loaded
  ...
  for (const [index, op] of ops.entries()) {
    ...
    if (name === 'insert_image' || name === 'insert_picture') {
      ...
      const call = HTML_TOOLS.has(name)
        ? { id: `Nawa-${index}`, name, input: htmlToolInput(doc, name, op) }
        : SIDE_TOOLS.has(name)
          ? { id: `Nawa-${index}`, name, input: sideToolInput(doc, name, op) }
          : { id: `Nawa-${index}`, name: 'apply_ops', input: { ops: [op] } }
