    ...
    switch (op.op) {
        ...
        if (!draft) {
          ...
          if (existing!.hasCriteria) {
            reject(
              i,
              op.op,
              `${sheet}'s filter already has criteria the CLI cannot re-save; run set_filter on the range first (it clears them) or use the Nawa app`,
            )
          }
      ...
      case 'edit_chart': {
        ...
        if (op.seriesData !== undefined) {
          reject(i, op.op, 'seriesData (repointing a series at new cells) needs the Nawa app')
        }
