...
const ABOUT =
  'Nawa: create, read, convert, edit and render Office documents locally (docx, xlsx, pptx, pdf, md, html, csv). The app need not be running; render, convert-to-pdf and create_pdf start a hidden Nawa process for a few seconds.'
const WORKFLOW = [
...
export const INSTRUCTIONS = [
  ABOUT,
  'Paths are absolute, or relative to the working directory the server was started in. Only search, image and media send data off the machine, to the provider configured in Nawa.',
  ...WORKFLOW,
  'A file open in a Nawa tab is not written unless force is set. Do not call open unless the user asks to see the file.',
].join('\n')
...
export function remoteInstructions(baseUrl: string): string {
  ...
  return [
    ...
    'Omit out: the file a tool writes comes back in the result as output_url (download it with curl -o) and, when small, as an embedded resource with the bytes. Only search, image and media send data to the cloud provider configured in Nawa.',
    ...WORKFLOW,
  ].join('\n')
}
