...
const GUI_OPEN =
  'A file open in a Nawa tab is refused without force (the editor would overwrite the edit on its next save).'
...
export const TOOLS: ToolSpec[] = [
  ...
  },
  {
    ...
    command: 'convert',
    description:
      'Convert a document to another format with the Nawa engines (pdf, docx, xlsx, pptx, md, html, csv; the `to` description lists the routes). Targets that need layout (to pdf, docx to html, html to docx) start a hidden Nawa process for 1-6 s. Output defaults to the input name with the new extension.',
  ...
  },
  {
    ...
    command: 'create',
    description:
      'Print a document (md, html, docx, xlsx, pptx) to a new PDF with the Nawa renderer, in a hidden Nawa process.',
    fixed: ['--type', 'pdf'],
    options: [
  ...
  },
  {
    ...
    images: true,
    description:
      'One PNG per slide (960x540 at scale 1 for 16:9), written to a directory and returned as images so you can look at the pages. Starts a hidden Nawa process (a few seconds). Use after building or editing a deck; slide renders one page.',
    positionals: [{ key: 'file', description: 'path of the .pptx' }],
    options: [
  ...
  },
  {
    ...
    images: true,
    description:
      'One PNG per page of a document (docx, xlsx, pptx, pdf, md, html) as the Nawa renderer lays it out, written to a directory and returned as images. Starts a hidden Nawa process. page renders one page; grid adds a contact sheet of every page.',
    positionals: [{ key: 'file', description: 'path of the document' }],
    options: [
      { key: 'out', description: 'directory for the PNGs (required)', required: true },
      'page',
  ...
  },
  {
    ...
    readOnly: true,
    description:
      'Which cloud features are configured in Nawa: web search, image search, image generation, media understanding. Check once before planning photos or live facts; everything else runs locally.',
  },
  {
    name: 'search',
    command: 'search',
    ...
    readOnly: true,
    description:
      'Web search, or image search with images=true, through the provider configured in Nawa (the query leaves the machine). Image results carry imageUrl, width and height for addPicture / insert_image.',
  ...
  },
  {
    ...
    openWorld: true,
    description:
      'Generate an image from a prompt with the provider configured in Nawa and save it; the result names the real format. ref images steer edits (background removal, upscale).',
  ...
  },
  {
    ...
    command: 'open',
    description:
      'Open a document in the Nawa app for the user (starts the app if needed), optionally selecting a slide, element, block, range or page. Only when the user asks to see the file: an open tab makes later *_apply calls refuse to write.',
    positionals: [{ key: 'file', description: 'path of the document' }],
