...
/// <Documents>/Nawa; now user-configurable via the `defaultSaveDir` key
...
// Optional host policy. Desktop callers retain the existing settings/fallback.
let scopedSaveDir: (() => string) | null = null
export function setDefaultSaveDirProvider(provider: (() => string) | null): void {
  scopedSaveDir = provider
}
...
export function configuredDefaultSaveDir(app: PathProvider): string {
  if (scopedSaveDir) {
    const dir = scopedSaveDir()
    if (!isAbsolute(dir)) throw new Error('The host did not provide an absolute save directory')
    accessSync(dir, constants.W_OK)
    return dir
  }
  const settingsPath = join(app.getPath('userData'), 'app-settings.json')
  const fallback = join(app.getPath('documents'), 'Nawa')
