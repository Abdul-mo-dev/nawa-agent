...
describe('configuredDefaultSaveDir', () => {
  ...
  it('falls back to <Documents>/Nawa without a setting', () => {
    const userData = join(root, 'userData')
    const documents = join(root, 'Documents')
    mkdirSync(userData, { recursive: true })
    const app = {
    ...
    expect(configuredDefaultSaveDir(app)).toBe(join(documents, 'Nawa'))
    expect(existsSync(join(documents, 'Nawa'))).toBe(true)
  })
})
